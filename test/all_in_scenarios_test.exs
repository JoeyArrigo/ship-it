defmodule PokerServer.AllInScenariosTest do
  ...
  describe "critical all-in scenarios" do
    test "all-in with exact stack sizes creates single pot" do
      players = [
        player(1, 100, 0),  # Small blind
        player(2, 100, 1)   # Big blind
      ]
    ...
    test "short stack all-in creates proper side pot structure" do
      players = [
        player(1, 50, 0),   # Short stack
        player(2, 200, 1)   # Deep stack
      ]
      ...
      assert length(side_pots) == 1
      [pot] = side_pots
      assert pot.amount == 100  # Both players put in 50 each
      assert pot.eligible_players == MapSet.new([1, 2])
    end

    test "all-in with reraise creates side pot" do
      players = [
        player(1, 100, 0),  # Medium stack
        player(2, 300, 1)   # Deep stack
      ]
      ...
      assert length(side_pots) == 1
      [pot] = side_pots
      assert pot.amount == 200  # 100 from each player
      assert pot.eligible_players == MapSet.new([1, 2])
    end

    test "both players all-in with different stacks" do
      players = [
        player(1, 75, 0),   # Smaller stack
        player(2, 125, 1)   # Larger stack
      ]
    ...
    test "all-in prevents further betting action" do
      players = [
        player(1, 30, 0),   # Very short stack
        player(2, 500, 1)   # Deep stack
      ]
      ...
      assert :raise not in valid_actions  # Cannot raise when opponent is all-in
    end
    ...
    test "min raise still applies with all-in constraint" do
      players = [
        player(1, 45, 0),   # Can only make small all-in
        player(2, 500, 1)   # Deep stack
      ]
      ...
      assert player_1.chips == 0  # All chips committed
