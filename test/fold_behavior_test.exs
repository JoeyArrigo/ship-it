defmodule PokerServer.FoldBehaviorTest do
  ...
  describe "folded players cannot win hands" do
    test "folded player excluded from showdown", %{game_pid: game_pid} do
      ...
      folded_players = MapSet.new(["player1"])  # player1 has folded
      
      # Call showdown with folded players
      result = GameState.showdown(game_state, folded_players)
      ...
      assert player1_result.chips == 1000  # Unchanged from folding
    end
    ...
    test "only non-folded players eligible for side pots" do
      ...
      {:ok, round_after_fold} = BettingRound.process_action(betting_round, first_active.id, {:fold})
      
      second_active = BettingRound.get_active_player(round_after_fold)
      {:ok, round_after_allin} = BettingRound.process_action(round_after_fold, second_active.id, {:all_in})
      
      third_active = BettingRound.get_active_player(round_after_allin)
      {:ok, final_round} = BettingRound.process_action(round_after_allin, third_active.id, {:call})
  ...
  describe "folded players cannot act" do
    ...
    test "betting skips folded players", %{game_pid: game_pid} do
      ...
      # First player folds
      {:ok, :action_processed, state_after_fold} = GameServer.player_action(game_pid, first_active.id, {:fold})
  ...
  describe "early hand termination" do
    ...
    test "hand continues with multiple remaining players", %{} do
      ...
      active_players = length(state_after_fold.betting_round.players) - 
  ...
  describe "integration with existing game flow" do
    test "fold preserves tournament elimination logic" do
      ...
      players = [{"rich_player", 1000}, {"poor_player", 25}]  # Poor player can only survive a few hands
      {:ok, game_id} = PokerServer.GameManager.create_game(players)
      [{game_pid, _}] = Registry.lookup(PokerServer.GameRegistry, game_id)
    ...
    test "fold works correctly with all-in scenarios" do
      ...
      assert state_after_fold.phase == :preflop_betting
      remaining_players = length(state_after_fold.betting_round.players) - 
                         MapSet.size(state_after_fold.betting_round.folded_players)
