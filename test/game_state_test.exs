defmodule PokerServer.GameStateTest do
  ...
  describe "new/1" do
    ...
    test "initializes deck" do
      ...
      # Short deck
      assert length(game_state.deck) == 36
    end
  end
  ...
  describe "start_hand/1" do
    test "transitions to dealing phase and deals hole cards" do
      ...
      # 36 - 12 hole cards
      assert length(updated_state.deck) == 24
    ...
    test "handles small blind player with insufficient chips" do
      ...
      players = [
        ...
        player(1, 5),
        # Position 1 - will be big blind
        player(2, 1500),
        # Position 2 - will be button
        player(3, 1500)
      ]
      ...
      game_state =
        ...
        |> Map.put(:small_blind, 10)
        |> Map.put(:big_blind, 20)
        # Button advances from 1 to 2, making player 1 SB
        |> Map.put(:button_position, 1)
      ...
      # 1500 - 20
      assert bb_player.chips == 1480

      # Pot should have partial small blind + full big blind
      # 5 + 20
      assert updated_state.pot == 25
    end
    ...
    test "handles big blind player with insufficient chips" do
      ...
      game_state =
        ...
        |> Map.put(:small_blind, 10)
        |> Map.put(:big_blind, 20)
        # Button advances from 2 to 0, making player 2 SB, player 3 BB
    ...
    test "handles both blind players with insufficient chips" do
      ...
      players = [
        # Position 0 - will be button
        player(1, 1500),
        # Position 1 - will be small blind, insufficient
        player(2, 5),
        # Position 2 - will be big blind, insufficient
        player(3, 12)
      ]
    ...
    test "handles exact blind amounts (boundary case)" do
      ...
      players = [
        # Position 0 - will be button
        player(1, 1500),
        # Position 1 - will be small blind, exactly 10
        player(2, 10),
        # Position 2 - will be big blind, exactly 20
        player(3, 20)
      ]
    ...
    test "wraps button position around table" do
      ...
      game_state =
        GameState.new(players)
        # Last position
        |> Map.put(:button_position, 5)

      updated_state = GameState.start_hand(game_state)

      # Wraps to first position
  ...
  describe "deal_flop/1" do
    test "deals 3 community cards and transitions to flop" do
      ...
      # 24 - 1 burn - 3 flop
      assert length(updated_state.deck) == 20
    end
  ...
  describe "deal_turn/1" do
    test "deals 1 more community card and transitions to turn" do
      ...
      # 20 - 1 burn - 1 turn
      assert length(updated_state.deck) == 18
    end
  end
  ...
  describe "deal_river/1" do
    test "deals final community card and transitions to river" do
      ...
      # 18 - 1 burn - 1 river
      assert length(updated_state.deck) == 16
    end
  end
    ...
    test "determines winners and distributes pot" do
      ...
      players = [
        ...
        %Player{
          id: 2,
          chips: 1480,
          hole_cards: [%Card{rank: :seven, suit: :clubs}, %Card{rank: :six, suit: :clubs}]
        }
      ]
  ...
  describe "eliminate_players/1" do
    ...
    test "adjusts button position when players eliminated" do
      players = [
        ...
        # Position 2
        player(3, 1000),
        # Position 3 - eliminated
        player(4, 0)
      ]

      game_state = %GameState{
        players: players,
        # Button on eliminated player
        button_position: 3,
        phase: :hand_complete,
        hand_number: 1,
        deck: [],
      ...
      # Only 2 players left, positions 0 and 1
      assert updated_state.button_position in [0, 1]
    end
    ...
    test "handles button on eliminated player with multiple consecutive eliminations" do
      ...
      # 3 players left
      assert updated_state.button_position in [0, 1, 2]
    ...
    test "handles all but one player eliminated" do
      ...
      players = [
        ...
        player(3, 0),
        # Position 3 - eliminated (button)
        player(4, 0),
        # Position 4 - eliminated
        player(5, 0),
        # Position 5 - eliminated
        player(6, 0)
      ]
    ...
    test "preserves button position when button player survives elimination" do
      ...
      players = [
        # Position 0 - eliminated
        player(1, 0),
        # Position 1 - survives (button)
        player(2, 1500),
        # Position 2 - eliminated
        player(3, 0),
        # Position 3 - survives
  ...
  describe "reset_for_next_hand/1" do
    test "resets hand-specific state while preserving tournament state" do
      ...
      # New shuffled deck
      assert length(updated_state.deck) == 36
